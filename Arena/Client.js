...
function Tank()
{
    ...
    this.getDirection = function(){ this.m_direction; };
    this.setDirection = function(){ this.m_direction = dir; };
    ...
    this.update = function()
    {
       ...
       {
           this.m_direction = this.calcLineDirectionTo(your_base[0], your_base[1]);
           this.shoot();
       }
...
function OnPlaceTankRequest()
{
    ...
    $target = {};
    $target[TEAM_1] = [[20, 8], [20, 7], [20, 6], [20, 5]];
    ...
    $path[TEAM_1][1] = [[20, 1]];
    $path[TEAM_1][2] = [[20, 1]];
    $path[TEAM_1][3] = [[18, 1], [17, 1], [20, 1], [20, 5]];
...
function Update()
{
    ...
    for (var i = 0; i < NUMBER_OF_TANK; i++)
    {
        ...
        if (t && t.m_HP > 0)
        {
            ...
            {
                // get next position
                var next = t.path[t.path.length-1];
                // Get next position is current is my position
                if(next[0] == t.getX() && next[1] == t.getY())
                {
                    t.path.pop();
                    //get next target
                    next = t.path[t.path.length-1];
                }
                else
                {
                    t.jumpTo(next[0], next[1]);
                    break;
                }
            }
